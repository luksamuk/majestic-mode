...
(defconst majestic-functions
  '((symbolp "x" "Check if object is symbol.")
    ...
    (vectorp "x" "Check if object is a vector.")
    ...
    ;; Vector functions
    (vector ". rest" "Create vector from elements.")
    (vec-type "vec" "Get commonest type of vector elements.")
    (vec-coerce "type vec" "Coerce vector elements to `type'")
    (vec-push "vec x" "Push `x' to end of vector.")
    (vec-set "vec pos x" "Replace vector element at position with `x'.")
    (vec-pop "vec" "Remove and return element at end of vector.")
    (vec-deq "vec" "Remove and return element at beginning of vector.")
    (vec-length "vec" "Retrieve length of vector.")
    (vec-at "vec pos" "Return vector element at position.")
    (vec-remove "vec pos" "Remove and return vector element at position.")
    (vec-insert "vec pos x" "Insert element at vector position.")

    ;; Number functions
    (number-coerce "subtype x" "Coerce number `x' to `subtype'.")
    ...
    (write "x stream" "Write object `x' to `stream'.")
    (read "stream" "Read and parse object from `stream'.")
    (read-char "stream" "Read single character from `stream'.")
    (peek-char "stream" "Read single character from `stream' without consuming it.")
    (write-char "c stream" "Write character to `stream'.")
    (write-string "str stream" "Write string to `stream'.")
...
(defconst majestic-keywords
  '((cond ". clauses" "Perform control flow for more than one predicate.")
    ...
    (while "pred . body" "Execute `body' while `pred' is true.")
    (unwind-protect "expr cleanup" "Execute `expr' and run `cleanup' regardless of errors in `expr' execution.")
    (until "pred . body" "Execute `body' while `pred' is false.")
    (repeat "n . body" "Execute `body', `n' times.")
    (with-open-stream "(sym dir file) . body" "Open stream to `file' with direction `dir', bound to symbol `sym', then close it after executing `body'."))
